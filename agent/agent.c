...
agent_to_ice_compatibility(NiceAgent *agent) {
    return agent->compatibility == NICE_COMPATIBILITY_GOOGLE ? STUN_USAGE_ICE_COMPATIBILITY_GOOGLE : agent->compatibility == NICE_COMPATIBILITY_MSN ? STUN_USAGE_ICE_COMPATIBILITY_MSN
                                                                                             : agent->compatibility == NICE_COMPATIBILITY_WLM2009   ? STUN_USAGE_ICE_COMPATIBILITY_MSICE2
                                                                                             : agent->compatibility == NICE_COMPATIBILITY_OC2007    ? STUN_USAGE_ICE_COMPATIBILITY_MSN
                                                                                             : agent->compatibility == NICE_COMPATIBILITY_OC2007R2  ? STUN_USAGE_ICE_COMPATIBILITY_MSICE2
...
agent_to_turn_compatibility(NiceAgent *agent) {
    return agent->compatibility == NICE_COMPATIBILITY_GOOGLE ? STUN_USAGE_TURN_COMPATIBILITY_GOOGLE : agent->compatibility == NICE_COMPATIBILITY_MSN ? STUN_USAGE_TURN_COMPATIBILITY_MSN
                                                                                              : agent->compatibility == NICE_COMPATIBILITY_WLM2009   ? STUN_USAGE_TURN_COMPATIBILITY_MSN
                                                                                              : agent->compatibility == NICE_COMPATIBILITY_OC2007    ? STUN_USAGE_TURN_COMPATIBILITY_OC2007
                                                                                              : agent->compatibility == NICE_COMPATIBILITY_OC2007R2  ? STUN_USAGE_TURN_COMPATIBILITY_OC2007
                                                                                                                                                     : STUN_USAGE_TURN_COMPATIBILITY_RFC5766;
}

NiceTurnSocketCompatibility
agent_to_turn_socket_compatibility(NiceAgent *agent) {
    return agent->compatibility == NICE_COMPATIBILITY_GOOGLE ? NICE_TURN_SOCKET_COMPATIBILITY_GOOGLE : agent->compatibility == NICE_COMPATIBILITY_MSN ? NICE_TURN_SOCKET_COMPATIBILITY_MSN
                                                                                               : agent->compatibility == NICE_COMPATIBILITY_WLM2009   ? NICE_TURN_SOCKET_COMPATIBILITY_MSN
                                                                                               : agent->compatibility == NICE_COMPATIBILITY_OC2007    ? NICE_TURN_SOCKET_COMPATIBILITY_OC2007
                                                                                               : agent->compatibility == NICE_COMPATIBILITY_OC2007R2  ? NICE_TURN_SOCKET_COMPATIBILITY_OC2007
...
nice_agent_new_full(GMainContext *ctx,
    ...
    NiceAgent *agent = g_object_new(NICE_TYPE_AGENT,
                                    ...
                                    "nomination-mode", (flags & NICE_AGENT_OPTION_REGULAR_NOMINATION) ? NICE_NOMINATION_MODE_REGULAR : NICE_NOMINATION_MODE_AGGRESSIVE,
...
void agent_signal_initial_binding_request_received(NiceAgent *agent,
                                                   NiceStream *stream) {
...
void agent_signal_new_selected_pair(NiceAgent *agent, guint stream_id,
    ...
    if (nice_debug_is_enabled()) {
        ...
        nice_debug("Agent %p: Local selected pair: %d:%d %s %s %s:%d %s",
                   ...
                   lcandidate->transport == NICE_CANDIDATE_TRANSPORT_TCP_ACTIVE ? "TCP-ACT" : lcandidate->transport == NICE_CANDIDATE_TRANSPORT_TCP_PASSIVE ? "TCP-PASS"
                                                                                      : lcandidate->transport == NICE_CANDIDATE_TRANSPORT_UDP               ? "UDP"
                                                                                                                                                            : "???",
                   ip, port, lcandidate->type == NICE_CANDIDATE_TYPE_HOST ? "HOST" : lcandidate->type == NICE_CANDIDATE_TYPE_SERVER_REFLEXIVE ? "SRV-RFLX"
                                                                             : lcandidate->type == NICE_CANDIDATE_TYPE_RELAYED                ? "RELAYED"
                                                                             : lcandidate->type == NICE_CANDIDATE_TYPE_PEER_REFLEXIVE         ? "PEER-RFLX"
                                                                                                                                              : "???");
        ...
        nice_debug("Agent %p: Remote selected pair: %d:%d %s %s %s:%d %s",
                   ...
                   rcandidate->transport == NICE_CANDIDATE_TRANSPORT_TCP_ACTIVE ? "TCP-ACT" : rcandidate->transport == NICE_CANDIDATE_TRANSPORT_TCP_PASSIVE ? "TCP-PASS"
                                                                                      : rcandidate->transport == NICE_CANDIDATE_TRANSPORT_UDP               ? "UDP"
                                                                                                                                                            : "???",
                   ip, port, rcandidate->type == NICE_CANDIDATE_TYPE_HOST ? "HOST" : rcandidate->type == NICE_CANDIDATE_TYPE_SERVER_REFLEXIVE ? "SRV-RFLX"
                                                                             : rcandidate->type == NICE_CANDIDATE_TYPE_RELAYED                ? "RELAYED"
                                                                             : rcandidate->type == NICE_CANDIDATE_TYPE_PEER_REFLEXIVE         ? "PEER-RFLX"
                                                                                                                                              : "???");
    }
...
nice_agent_set_relay_info(NiceAgent *agent,
    ...
    nice_debug("Agent %p: added relay server [%s]:%d of type %d to s/c %d/%d "
               "with user/pass : %s -- %s",
               agent, server_ip, server_port, type,
                ...
                while (res == HOST_CANDIDATE_CANT_CREATE_SOCKET ||
                    ...
                    if (nice_debug_is_enabled()) {
                        ...
                        nice_debug("Agent %p: s%d/c%d: creation of host candidate "
                                   ...
                                   transport == NICE_CANDIDATE_TRANSPORT_TCP_ACTIVE ? 0 : current_port,
       ...
       */
           ...
           */
                    nice_debug("Agent %p : we have a TCP candidate, but conncheck "
                               "has started already in aggressive mode, ignore it",
                               agent);
                    goto errors;
                } else {
...
agent_recv_message_unlocked(
    ...
    if (nice_debug_is_verbose()) {
        ...
        nice_debug_verbose("%s: Agent %p : Packet received on local socket %p "
                           "(fd %d) from [%s]:%u (%" G_GSSIZE_FORMAT " octets).",
                           G_STRFUNC, agent,
...
agent_consume_next_rfc4571_chunk(NiceAgent *agent, NiceComponent *component,
    ...
    if (messages != NULL) {
        ...
        bytes_copied = append_buffer_to_input_messages(agent->bytestream_tcp,
                                                       messages, n_messages, iter, component->rfc4571_buffer + component->rfc4571_frame_offset + component->rfc4571_frame_size - bytes_unconsumed,
                                                       bytes_unconsumed);
...
gsize memcpy_buffer_to_input_message(NiceInputMessage *message,
    ...
    if (buffer_length > 0) {
        g_warning("Dropped %" G_GSIZE_FORMAT " bytes of data from the end of "
                  ...
                  "message %p",
                  buffer_length, buffer - message->length,
    ...
    if (component->selected_pair.local != NULL) {
        if (nice_debug_is_enabled()) {
            ...
            nice_debug_verbose("Agent %p : s%d:%d: sending %u messages to "
                               "[%s]:%d",
                               agent, stream_id, component_id, n_messages, tmpbuf,
     ...
     * I/O callback directly from the pseudo-TCP receive buffer. */
       ...
       * will be parsed in-place. */
            ...
            nice_debug_verbose("%s: %p: received %d valid messages with %" G_GSSIZE_FORMAT
                               " bytes",
                               G_STRFUNC, agent, retval, local_message.length);
       ...
       * when in reliable mode. */
        ...
        while (has_io_callback ||
            ...
            if (msg->length > 0) {
                nice_debug_verbose("%s: %p: received a valid message with %" G_GSIZE_FORMAT " bytes", G_STRFUNC, agent, msg->length);
                if (has_io_callback) {
        ...
        while (has_io_callback) {
            ...
            if (retval == RECV_SUCCESS) {
                nice_debug_verbose("%s: %p: received a valid message with %" G_GSSIZE_FORMAT
                                   " bytes",
                                   G_STRFUNC, agent, local_message.length);
...
nice_agent_consent_lost(
    ...
    if (!agent->consent_freshness) {
        g_warning("Agent %p: Attempt made to signal consent lost for "
                  ...
                  "for this agent. Ignoring request",
