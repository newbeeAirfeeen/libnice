...
socket_source_detach(SocketSource *source) {
    nice_debug("Detaching source %p (socket %p, FD %d) from context %p",
               ...
               (source->socket->fileno != NULL) ? g_socket_get_fd(source->socket->fileno) : 0,
 ...
 */
void nice_component_update_selected_pair(NiceAgent *agent, NiceComponent *component, const CandidatePair *pair) {
    NiceStream *stream;
