...
nice_address_equal(const NiceAddress *a, const NiceAddress *b) {
    ...
    switch (a->s.addr.sa_family) {
        case AF_INET:
            return (a->s.ip4.sin_addr.s_addr == b->s.ip4.sin_addr.s_addr) && (a->s.ip4.sin_port == b->s.ip4.sin_port);

        case AF_INET6:
            return IN6_ARE_ADDR_EQUAL(&a->s.ip6.sin6_addr, &b->s.ip6.sin6_addr) && (a->s.ip6.sin6_port == b->s.ip6.sin6_port) && (a->s.ip6.sin6_scope_id == 0 || b->s.ip6.sin6_scope_id == 0 || (a->s.ip6.sin6_scope_id == b->s.ip6.sin6_scope_id));
...
nice_address_equal_no_port(const NiceAddress *a, const NiceAddress *b) {
    ...
    switch (a->s.addr.sa_family) {
        ...
        case AF_INET6:
            return IN6_ARE_ADDR_EQUAL(&a->s.ip6.sin6_addr, &b->s.ip6.sin6_addr) && (a->s.ip6.sin6_scope_id == 0 || b->s.ip6.sin6_scope_id == 0 ||
