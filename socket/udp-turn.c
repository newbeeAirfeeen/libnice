...
void nice_udp_turn_socket_cache_realm_nonce(NiceSocket *sock,
                                            StunMessage *msg) {
    g_mutex_lock(&mutex);
...
guint nice_udp_turn_socket_parse_recv_message(NiceSocket *sock, NiceSocket **from_sock,
...
priv_send_channel_bind(UdpTurnPriv *priv, uint16_t channel,
    ...
    if (priv->username != NULL && priv->username_len > 0 &&
        ...
        if (stun_message_append_bytes(&msg->message, STUN_ATTRIBUTE_REALM,
                                      priv->cached_realm, priv->cached_realm_len) != STUN_MESSAGE_RETURN_SUCCESS) {
            g_free(msg);
            return 0;
        }

        if (stun_message_append_bytes(&msg->message, STUN_ATTRIBUTE_NONCE,
                                      priv->cached_nonce, priv->cached_nonce_len) != STUN_MESSAGE_RETURN_SUCCESS) {
            g_free(msg);
            return 0;
        }
    }
...
priv_add_channel_binding(UdpTurnPriv *priv, const NiceAddress *peer) {
    ...
    if (priv->compatibility == NICE_TURN_SOCKET_COMPATIBILITY_DRAFT9 ||
        ...
        if (stun_message_append_addr(&msg->message,
                                     ...
                                     &sa.addr, sizeof(sa)) != STUN_MESSAGE_RETURN_SUCCESS) {
            g_free(msg);
            return FALSE;
        }
